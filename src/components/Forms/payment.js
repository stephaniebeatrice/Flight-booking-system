...
export const Payment = ({ setSelectedTab }) => {
	const {
		seatsSelected,
		bookingUserInfo: passengers,
		pendingBooking
	} = useSelector((state) => state.bookingReducer)
	const [validation, setValidation] = useState({
		cardHolderFullName: true,
		cardNumber: true,
		cvv: true
	})
	...
	const submitHandler = async (e) => {
		try {
			...
			if (!isValid) return
			if (
				+pendingBooking.passengers !== passengers.length ||
				seatsSelected.length !== +pendingBooking.passengers
			) {
				return setShowPop(true)
			}

			const res = await fetch(
				'https://flight-booking-server-3zln.vercel.app/flight/create-booking',
				{
					method: 'POST',
					body: JSON.stringify(
						jsonObj(pendingBooking, passengers, seatsSelected)
					),
	...
	return (
		<div className="form-content">
						...
						<div className="fieldset half">
							<div className="field">
								...
								<input
									...
									style={{
										border: !validation.cardHolderFullName
											? '1px solid red'
											: ''
									}}
							...
							<div className="field">
								...
								<input
									...
									name="cardNumber"
									style={{
										border: !validation.cardNumber ? '1px solid red' : ''
									}}
								...
								<div className="fieldset quarter">
									...
									<div className="field">
										...
										<input
											style={{
												width: '100%',
												border: !validation.cvv ? '1px solid red' : ''
											}}
											id="cvv"
											className="form-control"
											placeholder="415"
			...
			{showPop && (
				<Pop
					text={
						'Please fill in the previous forms details before making payments'
					}
					okText={'Ok'}
	...
	return data.map((card, index) => (
		<div key={index} className="card-logos">
			<img
				...
				alt={'card'}
				style={
					data.length === index + 1 ? { height: ' 64px', width: '74px' } : {}
				}
			/>
		</div>
	))
