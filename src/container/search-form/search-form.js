	...
	const handleSubmit = async (event) => {
		try {
			...
			const res = await fetch(
				'https://flight-booking-server-rust.vercel.app/flight/search',
				{
					method: 'POST',
					body: JSON.stringify({
							...
							<tbody>
								{searchResult.map((flight, index) => {
									const match =
										...
										new Date(form.departureTime).toLocaleDateString()
									return (
										<tr key={index.toString()}>
											...
											<td>${flight.price}</td>
											<td style={{ color: match ? 'green' : 'orange' }}>
												{match ? 'Match!' : 'Recommended'}
											</td>
											<td>
												<button onClick={() => handleButtonClick(flight)}>
													Book
												</button>
											</td>
										</tr>
