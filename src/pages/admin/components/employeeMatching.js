...
export const EmployeeMatching = () => {
  ...
  const handlePrintSuccessfulMatches = () => {
    const successfulMatches = applicants.filter(applicant => applicant.status === "Approved");
    printSuccessfulApplicants(successfulMatches);
  };

  const printSuccessfulApplicants = successfulMatches => {
    const printWindow = window.open("", "_blank");
    printWindow.document.write("<html><head><title>Approved Applicants</title></head><body>");
    printWindow.document.write('<h1 style="text-align: center;">Approved Applicants</h1>');
    printWindow.document.write('<table style="border-collapse: collapse; width: 100%;">');
    printWindow.document.write("<thead><tr><th style='border: 1px solid black; padding: 5px;'>Applicant ID</th>");
    printWindow.document.write("<th style='border: 1px solid black; padding: 5px;'>Full Names</th>");
    printWindow.document.write("<th style='border: 1px solid black; padding: 5px;'>Applied Job</th></tr></thead>");
    printWindow.document.write("<tbody>");
    successfulMatches.forEach(applicant => {
      printWindow.document.write("<tr>");
      printWindow.document.write(`<td style='border: 1px solid black; padding: 5px;'>${applicant.id}</td>`);
      printWindow.document.write(`<td style='border: 1px solid black; padding: 5px;'>${applicant.name}</td>`);
      printWindow.document.write(`<td style='border: 1px solid black; padding: 5px;'>${applicant.appliedJob}</td>`);
      printWindow.document.write("</tr>");
    });
              ...
              {applicants.map(applicant => (
                <tr key={applicant.id}>
                  ...
                  <td>
                    ...
                    <button onClick={handlePrintSuccessfulMatches} className="button">
                      Print
                    </button>
                  </td>
                </tr>
